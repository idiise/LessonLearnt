...
# Partie SCA PMD2 2018 ----------------------------------------------------
# Codebook base ménage
BasePDM2CoinjointSoudure2018_Menage<- to_factor(BasePDM2CoinjointSoudure2018_Menage)
codebook_PDM2_2018 <- var_label(BasePDM2CoinjointSoudure2018_Menage)
codebook_PDM2_2018 <- as.data.frame(do.call(rbind,codebook_PDM2_2018))
codebook_PDM2_2018 <- codebook_PDM2_2018 %>% rownames_to_column()

write_xlsx(codebook_PDM2_2018, "Codebook/codebook_PDM2_2018.xlsx")

BasePDM2CoinjointSoudure2018_Menage <- BasePDM2CoinjointSoudure2018_Menage %>% 
  mutate(
    caa2 = replace_na(caa2, 0), #céréales
    FCSPulse = replace_na(cac2, 0), #Légumineuses/noix
    cab2 = replace_na(cab2, 0), #tubercules
...
funModeling::freq(BasePDM2CoinjointSoudure2018_Menage, "FCSCat28") 
# Acceptable = 38.48
# Limite =  30.78
# Pauvre =  30.74

# Partie LHCS PDM2 2018 ---------------------------------------------------

#  stratégies de stress
BasePDM2CoinjointSoudure2018_Menage <- BasePDM2CoinjointSoudure2018_Menage %>% mutate( stress_coping =
                                                                                         case_when(
                                                                                           ss9x1 %in% c("Oui","Non, parce que j’ai déjà vendu ces actifs ou mené cette activité au cours des 12 derniers mois et je ne peux pas c") ~ "Oui",
                                                                                           ss13x1 %in% c("Oui","Non, parce que j’ai déjà vendu ces actifs ou mené cette activité au cours des 12 derniers mois et je ne peux pas c") ~ "Oui",
                                                                                           ss15x1 %in% c("Oui","Non, parce que j’ai déjà vendu ces actifs ou mené cette activité au cours des 12 derniers mois et je ne peux pas c") ~ "Oui",
...
#  stratégies de crise
BasePDM2CoinjointSoudure2018_Menage <- BasePDM2CoinjointSoudure2018_Menage %>% mutate(crisis_coping = 
                                                                                        case_when(
                                                                                          ss10x1 %in% c("Oui") ~ "Oui",
                                                                                          ss11x1 %in% c("Oui") ~ "Oui",
                                                                                          ss12x1 %in% c("Oui") ~ "Oui",
...
#  stratégies d'urgence
BasePDM2CoinjointSoudure2018_Menage <- BasePDM2CoinjointSoudure2018_Menage %>% mutate(emergency_coping =
                                                                                        case_when(
                                                                                          ss8x1 %in% c("Oui") ~ "Oui",
                                                                                          ss14x1 %in% c("Oui") ~ "Oui",
                                                                                          ss16x1 %in% c("Oui") ~ "Oui",
...
BasePDM2CoinjointSoudure2018_Menage <- BasePDM2CoinjointSoudure2018_Menage %>% mutate(LhCSICat = case_when(
  emergency_coping == "Oui" ~ "StrategiesdeUrgence",
  crisis_coping == "Oui" ~ "StrategiesdeCrise",
  stress_coping == "Oui" ~ "StrategiesdeStress",
  TRUE ~ "Pasdestrategies"))

funModeling::freq(BasePDM2CoinjointSoudure2018_Menage, "LhCSICat")
